...
bool send_error_response(client& client, const communication_result& result) {
    std::array<uint8_t, 2> response = { request_type::error_response, result };
    std::array<uint8_t, as_collected_size(static_cast<nsize_int>(response.size()))> encoded_response;
    encode_with_size(
        response.begin(),
        response.end(),
...
bool process_client(client& client) {
    ...
    case request_type::register_data_definition: {
        ...
        if (std::find_if(
            ...
            return send_error_response(client, communication_result::already_registered);
        }

        if ((client.connection.collector.next_size() - connection::DATA_DEFINITION_DATA_START) % data::data_member_serialization_info::packed_size != 0) {
    ...
    case request_type::defined_data: {
        ...
        if (find_it == client.connection.data_definitions.end()) {
            return send_error_response(client, communication_result::not_registered);
        }
