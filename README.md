...
_A custom type is provided for specifying indices and counts._
...
_In certain languages, errors are values (C++) and in others, errors are exceptions (C#)._
...
Currently only supported on windows, the build system is VisualStudio. Most API calls are wrapped in a `truckconnect::platform` namespace for potential implementation on other platforms.
...
### Main Script: `truckconnect.py`

This will load the `scssdk_telemetry.json` file, and build the truckconnect telemetry data.

### C++ Script: `cpp.py`

This will load and build the truckconnect telemetry data, and generate C++ files.
...
The output directory of the generated files are determined by the `OUTPUT_FOLDER: Path` constant which by default is `generated.`
