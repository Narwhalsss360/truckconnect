...
int busy_request_test() {
    ...
    while (true) {
        ...
        for (int i = 0; i < trailer_connected.size(); i++) {
            debug_assert(communication_result::success == (result = request<metadata::trailer_channel_connected>(connection, trailer_connected[i])));
        }
